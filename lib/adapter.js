...
var doCursorJoin = require('waterline-cursor');
  ...
  var adapter = {
    ...
    join: function (connectionName, collectionName, criteria, cb) {
      ...
      doCursorJoin({
        ...
        $find: function (collectionIdentity, critera, cb) {
        ...
        $getPK: function (collectionIdentity) {
          ...
          var connectionObject = connections[connectionName];
          var collection = connectionObject.collections[collectionIdentity];
          console.log(collection.getPrimaryKey());
	...
	function marshalConfig(config) {
		return {
			...
			pool: {
				max: config.pool.max,
				min: config.pool.min,
				idleTimeoutMillis: config.pool.idleTimeout * 1000
			},
