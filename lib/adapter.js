...
var log = bunyan.createLogger({
  ...
  streams: [{
    path: 'c:\\code\\sails-mssqlserver\\build.log'
  }]
})
var debugging = false
/**
  ...
  var adapter = {
    ...
    connectConnection: function (connection, cb) {
      ...
      if (isPersistent && (!connections[connection].mssqlConnection || !connections[connection].mssqlConnection.connected)) {
        if (debugging) {
          console.log('Inside persistent and connected')
        }
        ...
        connections[connection].mssqlConnection.connect()
          ...
          .catch(function (err) {
            console.log('Error caught in persistent and connected', err)
            cb(err, null)
          })
        ...
        connections[connection].mssqlConnection[uniqId].connect()
          ...
          .catch(function (err) {
            if (debugging) {
              console.log('Inside connected caught', err)
            }
            cb(err, null)
          })
      } else {
