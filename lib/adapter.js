  ...
  var adapter = {
    ...
    connectConnection: function (conn, cb) {
      ...
      if (persistent && !connected) {
        connection.mssqlConnection = new mssql.ConnectionPool(marshalConfig(connection.config), cb)
      } else if (!persistent && !connected) {
        if (!assigned) {
          connection.mssqlConnection = []
        }

        connection.mssqlConnection[uniqId] = new mssql.ConnectionPool(marshalConfig(connection.config), function (err) {
          cb(err, uniqId)
        })
      } else {
        _.defer(cb)
