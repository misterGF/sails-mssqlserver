...
module.exports = (function() {
  ...
  var adapter = {
    ...
    connectConnection: function(conn, cb) {
      ...
      if (persistent) {
        ...
        connected =
          assigned &&
          connection.mssqlConnection[uniqId] &&
          connection.mssqlConnection[uniqId].connected
      }

      if (persistent && !connected) {
        connection.mssqlConnection = new mssql.Connection(
          marshalConfig(connection.config),
          cb
        )
        ...
        connection.mssqlConnection[uniqId] = new mssql.Connection(
          marshalConfig(connection.config),
          function(err) {
            cb(err, uniqId)
          }
      ...
      adapter.connectConnection(connection, function __UPDATE__(err, uniqId) {
        ...
        request.query(statement, function(err, recordset) {
          ...
          statement =
            ...
            tableName +
            ' SET ' +
            sql.updateCriteria(collection, values) +
            ' '
  ...
  function getSchemaName(connection, collection) {
    ...
    return collectionObject.meta && collectionObject.meta.schemaName
